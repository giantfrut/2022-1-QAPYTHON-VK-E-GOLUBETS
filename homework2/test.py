...
class TestTarget(BaseCase):
    ...
    def test_create_campaign(self, file_path, campaign_page):
        campaign_name = campaign_page.create_campaign(file_path)

        campaign_page.find((campaign_page.locators.CAMPAIGN_LOCATOR[0],
                            campaign_page.locators.CAMPAIGN_LOCATOR[1].format(name=campaign_name)))
